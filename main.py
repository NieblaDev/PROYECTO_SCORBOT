...
class AplicacionGUI:
    ...
    def configurar_interfaz(self):
        ...
        self.terminal_log = ScrolledText(right_frame, padding=5, height=15, font=("Consolas", 11), state=DISABLED, autohide=True)
    ...
    def escribir_en_terminal(self, texto):
        ...
        self.terminal_log.insert(END, f"{texto}\n")
        self.terminal_log.see(END)
        self.terminal_log.config(state=DISABLED)

    def limpiar_log(self):
        self.terminal_log.config(state=NORMAL)
        self.terminal_log.delete("1.0", END)
        self.terminal_log.config(state=DISABLED)
        ...
        if filepath:
            try:
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(self.terminal_log.get("1.0", END))
